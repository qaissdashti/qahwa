...
export async function notifyPayoutRequested({ creatorEmail, fullName, handle, amount, fee, bankName }) {
  // amount = gross requested; fee = payout fee charged; net = what they'll receive.
  const feeNum = Number(fee) || 0;
  const hasFee = feeNum > 0;
  const amt    = fmtKd(amount);                     // gross
  const feeStr = fmtKd(feeNum);                     // fee
  const netStr = fmtKd(Number(amount) - feeNum);    // net to creator
  ...
  // Subject + body lead with the NET they'll receive when a fee applies;
  // the breakdown (requested / fee / net) is spelled out below.
  const creatorSubject = `Payout request received — ${hasFee ? netStr : amt} KD`;
  const creatorText =
    `We received your payout request and will transfer it within 2-3 business days. ` +
    `You'll hear from us once it's sent.\n\n` +
    (hasFee
      ? `Amount requested:   ${amt} KD\n` +
        `Payout fee:         -${feeStr} KD\n` +
        `You'll receive:     ${netStr} KD\n\n`
      : `Amount: ${amt} KD\n\n`) +
    `— Qahwa ☕`;
  const creatorHtml =
    `<div style="font-family:'Plus Jakarta Sans',-apple-system,system-ui,sans-serif;max-width:520px;margin:0 auto;padding:24px;color:#0D0D0D">` +
    `<h2 style="margin:0 0 16px;font-family:'Plus Jakarta Sans',sans-serif">Payout request received</h2>` +
    `<p style="margin:0 0 16px;line-height:1.6">We received your payout request and will transfer it within ` +
      `<strong>2-3 business days</strong>. You'll hear from us once it's sent.</p>` +
    (hasFee
      ? `<div style="background:#EDE4FB;border:2px solid #0D0D0D;border-radius:12px;padding:14px 16px;margin:0 0 16px">` +
          `<table style="border-collapse:collapse;width:100%;font-size:14px">` +
            `<tr><td style="padding:4px 0;color:#666;width:55%">Amount requested</td><td style="padding:4px 0;font-weight:700;font-family:monospace">${escapeHtml(amt)} KD</td></tr>` +
            `<tr><td style="padding:4px 0;color:#666">Payout fee</td><td style="padding:4px 0;font-weight:700;font-family:monospace;color:#C00">-${escapeHtml(feeStr)} KD</td></tr>` +
            `<tr><td style="padding:6px 0 0;color:#666;border-top:1px solid #0D0D0D33">You'll receive</td><td style="padding:6px 0 0;font-weight:800;font-family:monospace;border-top:1px solid #0D0D0D33">${escapeHtml(netStr)} KD</td></tr>` +
          `</table>` +
        `</div>`
      : `<p style="margin:0 0 16px;line-height:1.6">Amount: <strong style="font-family:monospace">${escapeHtml(amt)} KD</strong></p>`) +
  ...
  const adminHtml =
    ...
    `<table style="border-collapse:collapse;margin:0 0 20px">` +
      ...
      `<tr><td style="padding:4px 12px 4px 0;color:#666">Handle</td><td style="padding:4px 0;font-weight:700">@${escapeHtml(safeHandle)}</td></tr>` +
      (hasFee
        ? `<tr><td style="padding:4px 12px 4px 0;color:#666">Requested</td><td style="padding:4px 0;font-weight:700;font-family:monospace">${escapeHtml(amt)} KD</td></tr>` +
          `<tr><td style="padding:4px 12px 4px 0;color:#666">Fee</td><td style="padding:4px 0;font-weight:700;font-family:monospace;color:#C00">-${escapeHtml(feeStr)} KD</td></tr>` +
          `<tr><td style="padding:4px 12px 4px 0;color:#666">Pay (net)</td><td style="padding:4px 0;font-weight:800;font-family:monospace">${escapeHtml(netStr)} KD</td></tr>`
  ...
  const waBody =
    ...
    `${safeName} (@${safeHandle}) — ${hasFee ? `${netStr} KD net (${amt} − ${feeStr} fee)` : `${amt} KD`}\n` +
    `Bank: ${safeBank}\n` +
    `Queue: ${link}`;
