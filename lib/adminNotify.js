...
export async function notifyPayoutPaid({ creatorEmail, fullName, amount, bankName, ibanMasked, lang }) {
  if (!creatorEmail) {
  ...
  const html =
    ...
    `<div style="background:#C8F55A;border:2px solid #0D0D0D;border-radius:12px;padding:16px 18px;margin:0 0 18px;box-shadow:3px 3px 0 #0D0D0D;text-align:center">` +
      `<div style="font-size:13px;font-weight:700;letter-spacing:0.04em;opacity:0.7;text-transform:uppercase;margin:0 0 4px">${amountLabel}</div>` +
      `<div style="font-family:monospace;font-size:32px;font-weight:900;margin:0">${escapeHtml(amt)} <span style="font-size:18px">${amtCurrency}</span></div>` +
    `</div>` +

    // Transfer details (lavender soft card, two rows)
    `<div style="background:#EDE4FB;border:2px solid #0D0D0D;border-radius:12px;padding:14px 16px;margin:0 0 18px">` +
      ...
      `<table style="border-collapse:collapse;width:100%;font-size:14px">` +
        `<tr><td style="padding:4px 0;color:#666;width:30%">${bankLabel}</td><td style="padding:4px 0;font-weight:700">${escapeHtml(safeBank)}</td></tr>` +
