...
export async function createCheckoutSession({ tipId, amount, creatorName, handle }) {
  ...
  const body = {
    ...
    interaction: {
      // TEMP DIAGNOSTIC: testing AUTHORIZE vs PURCHASE for acquirer profile — revert or confirm after Gulf Bank test
      operation: 'AUTHORIZE',
      returnUrl,
      merchant: {
    ...
    order: {
      ...
      currency: 'KWD',
      description: creatorName ? `Qahwa ☕ — ${creatorName}` : 'Qahwa ☕',
    },
  };
